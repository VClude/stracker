      ...
      <div class="inner">
        <loader :loading="loading">
          <div class="flex flex-3">
            ...
            <div class="flex-item">
              <h2>{{hero.name}}</h2>
              <img class="w3-round-large" :src="replaceImg(hero.image)" style="width:100%;" />
          ...
          <div class="w3-row-padding">
            <template v-for="(item) in hero.abilities">
              <div v-bind:key="item.id" class="w3-col s6 m3 l3 w3-padding-small w3-center">
                <img class="w3-round-large" :src="replaceImg(item.image)"  />
                <h4>
                  <router-link style="text-decoration: none;" :to="'/ability/' + item.id">{{item.name}}</router-link>
                </h4>
                <span class="w3-tag w3-xlarge w3-round-large" style="position: relative; top:-190px;left:50px;background-color:#f6755e;color:white !important;">{{Math.round(item.winRate * 100)}}%</span>
              </div>
            </template>
          </div>
          <br />
